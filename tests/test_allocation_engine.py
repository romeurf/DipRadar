...
def test_high_conviction_excludes_preprofit_bluechip():
    ...
    print(f"  ok:Pre-profit bluechip excluído de HC: cai em {d.category}")
...
def test_high_conviction_excludes_low_score_bluechip():
    ...
    print(f"  ok:Bluechip degradado sai de HC: {d.category}")
...
def test_growth_default_path():
    ...
    d = suggest_allocation(ctx)
    assert d.category == CAT_GROWTH, f"Esperava GROWTH, got {d.category}"
    print(f"  ok:Growth (fund>=55): {d.category}, €{d.amount_eur:.0f}")
...
def test_growth_target_20_for_win_strong():
    ...
    print(f"  ok:Growth WIN_STRONG ->+20%: {d.exit_rule}")
...
def test_flip_conflict_tech():
    ...
    print(f"  ok:Flip (CONFLICT_TECH): {d.category}, €{d.amount_eur:.0f}")
...
def test_pass_low_score_no_ml_bull():
    ...
    print(f"  ok:Pass (fund baixo, sem ML bull): {d.category}")
...
def test_score_multiplier_85_strong_max():
    ...
    print(f"  ok:Score 85+WIN_STRONG ->tier 1.5x: €{d.amount_eur:.0f}")
...
def test_score_multiplier_55_min_tier():
    ...
    print(f"  ok:Score 55 ->tier 0.3x: €{d.amount_eur:.0f}")
...
def test_sector_bonus_technology():
    ...
    print(f"  ok:Sector premium Tech: €{d_health.amount_eur:.0f} ->€{d_tech.amount_eur:.0f} (×{ratio:.2f})")
...
def test_preprofit_cap_half():
    ...
    print(f"  ok:Pre-profit cap ×0.5: €{d_clean.amount_eur:.0f} ->€{d_burnt.amount_eur:.0f}")
...
def test_concentration_cap_default_blocks_at_12pct():
    ...
    print(f"  ok:Concentration cap 12% blocks: €{d.amount_eur:.0f}, notes={d.notes[-1]}")
...
def test_concentration_cap_high_conviction_30pct():
    ...
    print(f"  ok:HC cap 30%: posição 25% ->€{d.amount_eur:.0f} (continua a acumular)")
...
def test_concentration_slowdown_above_8pct():
    ...
    print(f"  ok:Slowdown >8%: €{d_low.amount_eur:.0f} ->€{d_high.amount_eur:.0f}")
...
def test_regime_red_blocks_growth():
    ...
    print(f"  ok:Regime RED zera Growth: €{d.amount_eur:.0f}")
...
def test_regime_red_keeps_core():
    ...
    print(f"  ok:Regime RED permite Core: €{d.amount_eur:.0f}")
...
def test_regime_red_keeps_high_conviction():
    ...
    print(f"  ok:Regime RED permite HC: €{d.amount_eur:.0f}")
...
def test_regime_yellow_halves_growth():
    ...
    print(f"  ok:Regime YELLOW halves growth: €{d_g.amount_eur:.0f} ->€{d_y.amount_eur:.0f}")
...
def test_cash_cap():
    ...
    print(f"  ok:Cash cap: Core sugeria mais mas cap a €{d.amount_eur:.0f}")
...
def test_floor_minimum_ticket():
    ...
    print(f"  ok:Floor minimum: orçamento pequeno ->€0")
...
def test_backward_compat_dip_score_field():
    ...
    assert d.category == CAT_GROWTH, f"Esperava GROWTH, got {d.category}"
    assert d.amount_eur > 0
    print(f"  ok:Backward compat (dip_score só): {d.category}, €{d.amount_eur:.0f}")
...
def test_format_telegram_smoke():
    ...
    print(f"  ok:Telegram format smoke: {len(cases)} contextos, todos OK")
...
def main() -> int:
    ...
    if failed == 0:
        ...
        return 0
    print(f"\nFAILED — {failed}/{len(tests)} testes falharam")
    return 1
